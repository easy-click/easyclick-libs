 ...
 * @param path 图片路径
 * @return BufferedImage android的bitmap对象或者null
 */
...
ImageWrapper.prototype.findColorJ = function (image1, jsonFileName) {
    ...
    let data = readResString(jsonFileName + ".json");
    if (data == null || data == "") {
        return null;
    }
    data = JSON.parse(data);
    let firstColor = data['firstColor'];
 ...
 * @param w 剪裁宽度
 * @param h 剪裁高度
 * @return {BufferedImage} java的 BufferedImage 对象
 */
 ...
 * @param map map参数表
 * key分别为：
 * type : OCR类型，值分别为 ocrLite = ocrLite 模块
